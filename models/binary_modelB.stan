functions {
  ...
  real[] ode(real t, 
    ...
    real dydt[3];
    real beta_t = theta[1] * (1+x_r[7]*(t-x_i[1]));
    real p = atbswitch(t,x_r[1],x_r[2],x_r[3],x_r[4],x_r[5],x_r[6]);
    dydt[1] = - beta_t*y[1]*(y[2]+y[3]) + ((1-p)*theta[5]+p*theta[5]*(1-theta[2])+theta[3])*y[2] + 
              ((1-p)*theta[5]+p*theta[5]*theta[4]+theta[3])*y[3];
    dydt[2] = beta_t*y[1]*y[2] - ((1-p)*theta[5]+p*theta[5]*(1-theta[2])+theta[3])*y[2] - p*theta[5]*theta[2]*y[2];
    dydt[3] = beta_t*y[1]*y[3] + p*theta[5]*theta[2]*y[2] - ((1-p)*theta[5]+p*theta[5]*theta[4]+theta[3])*y[3];
    return(dydt);
  }
}
data {
...
transformed parameters {
  ...
  for(g in 1:G) {
    ...
    y[g] = integrate_ode_rk45(
      ...
      xis, // integer data
      1.0E-8, 1.0E-8, 1.0E3);
    for(i in 1:S) for(j in 1:C) y[g,i,j] =  y[g,i,j] + 1.0E-7; // avoid any negative values du to approximation of small values by adding some value greater than the ODE solver's precision
