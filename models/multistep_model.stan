functions {
  ...
  int[] x_i // [1] K
    ...
    dydt[1] = - theta[1]*y[1]*sum(y[2:(K+1)]) + 
...
parameters {
  ...
  ordered[G] tau_raw; // treatment rate on the log scale (by group, higher for group 2)
...
transformed parameters {
  real<lower=0> tau[G]; // treatment rate (by group)
